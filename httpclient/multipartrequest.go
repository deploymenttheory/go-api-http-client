...
func createMultipartRequestBody(files map[string][]string, formDataFields map[string]string, contentTypes map[string]string, formDataPartHeaders map[string]http.Header, log logger.Logger) (*bytes.Buffer, string, error) {
	...
	for fieldName, filePaths := range files {
		for _, filePath := range filePaths {
			if err := addFilePart(writer, fieldName, filePath, contentTypes, formDataPartHeaders, log); err != nil {
				return nil, "", err
			}
		}
	}
...
func addFilePart(writer *multipart.Writer, fieldName, filePath string, contentTypes map[string]string, formDataPartHeaders map[string]http.Header, log logger.Logger) error {
