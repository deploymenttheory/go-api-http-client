...
func (j *JamfAPIHandler) ConstructAPIResourceEndpoint(endpointPath string, log logger.Logger) string {
	url := fmt.Sprintf("https://%s%s%s", j.InstanceName, j.GetBaseDomain(), endpointPath)
...
func (j *JamfAPIHandler) GetBearerAuthEndpoint(log logger.Logger) string {
	urlBaseDomain := j.GetBaseDomain()
	url := fmt.Sprintf("https://%s%s%s", j.InstanceName, urlBaseDomain, BearerTokenEndpoint)
...
func (j *JamfAPIHandler) GetOAuthEndpoint(log logger.Logger) string {
	urlBaseDomain := j.GetBaseDomain()
	url := fmt.Sprintf("https://%s%s%s", j.InstanceName, urlBaseDomain, OAuthTokenEndpoint)
	j.Logger.Debug(fmt.Sprintf("Constructed %s API authentication URL", APIName), zap.String("URL", url))
	return url
}
